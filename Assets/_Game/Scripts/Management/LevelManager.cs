...
namespace _Game.Management
{
    ...
    public class LevelManager : MonoBehaviour
    {
        ...
        [SerializeField] private int _completeFee = 300;  // Fee for completing a level
        [SerializeField] private int _failFee = 20;       // Fee for failing a level
        ...
        //new code for calculating stars
        private int CalculateStars()
        {
            int health = _carDamageHandler.Health;
            int maxHealth = _carDamageHandler.MaxHealth; // This now uses the public property

            if (health == maxHealth)
            {
                return 3; // Perfect, no hits
            }
            else if (health >= maxHealth / 2) // More than or equal to half health
            {
                _completeFee = 200;
                return 2;
            }
            else if (health > 0) // At least one health point left
            {
                _completeFee = 100;
                return 1;
            }

            return 0; // No health left
        }
        ...
        /*/// <summary>
        /// new addition - this function will listen the level frame to complete building level then enables carcontroller
        /// </summary>
        public void OnFrameReady()
        {
            // Find the car in the scene
            CarController car = ServiceLocator.Get<CarController>();

            if (car != null)
            {
                // Tell the car to turn on its controls
                car.EnableControls(true);
                Debug.Log("Level Frame is ready. Player controls enabled!");
            }
        }*/
    }
}