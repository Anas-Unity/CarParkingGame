...
namespace _Game.LevelSystem
{
    ...
    public class Level : MonoBehaviour
    {
        ...
        [Tooltip("The time limit for this level in seconds. Set to 0 for no timer.")]
        ...
        /// <summary>
        /// new addition - Added the new public method that the LevelFrame will call.
        /// </summary>
        public void OnFrameReady()
        {
            Debug.Log("Level Frame is ready. Enabling player controls!");
            if (_carController != null)
            {
                _carController.EnableControls(true);
            }
        }


        /// <summary>
        /// Coroutine to handle the level's countdown timer.
        /// </summary>
        private IEnumerator CountdownRoutine()
        {
            _isTimerRunning = true;
            float currentTime = _levelTimeLimit;

            while (currentTime > 0)
            {
                if (!_isTimerRunning)
                {
                    // Exit if the timer was stopped (e.g., level was won)
                    yield break;
                }

                currentTime -= Time.deltaTime;

                // Here, you would update a UI element to show the time
                // For example: UIManager.Instance.UpdateTime(currentTime);

                yield return null; // Wait for the next frame
            }

            // If the loop finishes, time is up!
            Debug.Log("Time is up! Level failed.");
            _isTimerRunning = false;
            _levelManager.FailLevel(); // Assuming LevelManager has a method like this
        }
    }
}*/
...
{
    /// <summary>
    /// The Level class handles the initialization and management of a level, including a countdown timer.
    /// </summary>
    public class Level : MonoBehaviour
    {
        [Header("Game Data")]
        ...
        // --- REMOVED: The direct reference to the UI text is no longer needed here. ---
        ...
        private ParkingArea _parkingArea;
        private LevelManager _levelManager;
        private UIManager _uiManager; // --- NEW: A reference to the UIManager ---
        ...
        private IEnumerator InitializeDependencies()
        {
            // Wait for LevelManager and UIManager to be ready
            while (_levelManager == null || _uiManager == null)
            {
                _levelManager = ServiceLocator.Get<LevelManager>();
                _uiManager = ServiceLocator.Get<UIManager>(); // --- NEW: Get the UIManager ---
                yield return null;
            }
        }
        ...
        private void SubscribeToEvents()
        {
            if (!_levelManager) _levelManager = ServiceLocator.Get<LevelManager>();
            _levelManager.OnLevelComplete += HandleLevelEnd;
            _levelManager.OnLevelFail += HandleLevelEnd;
        }
        ...
        private void ValidateInitialReferences()
        {
            if (string.IsNullOrEmpty(_parkingAreaPrefabPath)) Debug.LogError("ParkingAreaPrefab path is missing.");
            if (_playerPoint == null) _playerPoint = transform.Find("PlayerPoint");
            if (_parkingAreaPoint == null) _parkingAreaPoint = transform.Find("ParkingAreaPoint");
            if (_playerPoint == null || _parkingAreaPoint == null) Debug.LogError("PlayerPoint or ParkingAreaPoint not found.");
            if (_levelFrame == null) _levelFrame = GetComponentInChildren<LevelFrame>();
            if (_levelFrame == null) Debug.LogError("LevelFrame component is missing.");
        }
        ...
        {
            _isTimerRunning = false;
            _uiManager?.ToggleTimerDisplay(false); // --- NEW: Hide the timer when the level ends ---
            if (_disposeDelay > 0)
            {
                DOVirtual.DelayedCall(_disposeDelay, Dispose);
            }
        ...
        public void OnFrameReady()
        {
            ...
            if (_levelTimeLimit > 0)
            {
                _uiManager?.ToggleTimerDisplay(true); 
                StartCoroutine(CountdownRoutine());
            }
            SubscribeToEvents();
        }
        ...
        private IEnumerator CountdownRoutine()
        {
            ...
            while (currentTime > 0)
            {
                ...
                // --- MODIFIED: Instead of updating text directly, call the UIManager ---
                if (_uiManager != null)
                {
                    _uiManager.UpdateTime(currentTime);
                }
